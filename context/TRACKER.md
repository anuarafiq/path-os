...
- Coach → Explore Paths highlight: asking the AI Coach to show/visualize a career path now opens `/explore` with that path pre-highlighted, same visual result as manually clicking a node and hitting "Set as destination". `navigateTo` tool gained an optional `targetRole`; `CareerPathExplorer` resolves it via `useSearchParams()`. See `.claude/ARCHITECTURE.md` Explore and Coach sections for the wiring and the mount-effect-deps gotcha this surfaced.

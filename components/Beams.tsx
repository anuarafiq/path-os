...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: ExtendMaterialConfig) {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    uniforms[key] =
      u !== null && typeof u === "object" && "value" in u ? (u as THREE.IUniform) : ({ value: u } as THREE.IUniform);
  });
...
const CanvasWrapper = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <Canvas dpr={[1, 2]} frameloop="always" className={`relative w-full h-full${className ? ` ${className}` : ""}`}>
    {children}
  </Canvas>
);
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height, spacing }, ref) => {
  ...
  useFrame((_, delta) => {
    // Standard r3f pattern: mutating a ref-derived material uniform inside useFrame's
    // per-frame callback, which runs outside React's render phase, same as an event handler.
